...
router.route("/").post(async (req, res) => {
  ...
  if (check) {
    return res.status(400).send({ message: "Test had already taken!" });
  }
  ...
  if (questions.data.response_code == 0) {
    ...
    return res.status(400).send({ message: "Sorry! Couldn't able to fetch test details. Try again!" });
  }
});
...
router.route("/submittest").post(async (req, res) => {
  ...
  resultEntry.save().then(() => res.send("Result Added!")).catch((err) => res.status(400).json("error : " + err));
});
...
router.route("/addtest").post(async (req, res) => {
  ...
  const newtest = new test({ pin, email, amount, topic, time, expiry, created });
  newtest.save().then(() => res.send("Test Added successfully!")).catch((err) => res.status(400).json("error : " + err));
});
